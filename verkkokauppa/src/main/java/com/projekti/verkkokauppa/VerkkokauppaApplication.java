...
public class VerkkokauppaApplication {
	...
	public CommandLineRunner verkkokauppaDemo(TuoteRepository trepository, ValmistajaRepository vrepository, UserRepository urepository) {
		return (args) -> {
			...
			Tuote t2 = new Tuote("Playstation 5", "Valkoinen", vrepository.findByNimi("Sony").get(0), 2020, 550.95);
			Tuote t3 = new Tuote("HP Omen 30L", "Musta", vrepository.findByNimi("HP").get(0), 2021, 2600.00);
			Tuote t4 = new Tuote("Sony Walkman", "Sininen", vrepository.findByNimi("Sony").get(0), 1979, 80.00);
			Tuote t5 = new Tuote("Asus ROG STRIX G16", "Musta", vrepository.findByNimi("Asus").get(0), 2022, 3299.99);
			Tuote t6 = new Tuote("Macbook Pro", "Hopea", vrepository.findByNimi("Apple").get(0), 2023, 1599.99);
